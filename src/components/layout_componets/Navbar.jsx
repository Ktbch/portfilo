...
export const Navbar = ({ menu, setMenu }) => {
    ...
    return (
        <nav className=" shadow-none  lg:shadow-sm  m-auto py-8 px-8 flex space-x-96  items-center dark:sahdow-none ">
            ...
            <div className={` ${!menu ? 'hidden translate-x-full' : 'absolute  translate-x-0'}  top-0.5 w-full py-20 h-screen right-[0.4%] bg-gradient-to-tr from-slate-300 to-white dark:from-color1 dark:to-slate-600 lg:block lg:translate-x-0 lg:right-[0] lg:w-auto  lg:h-auto lg:relative lg:p-0 lg:bg-none`}>
...
// `${!menu ? 'hidden translate-x-full' : 'absolute  translate-x-0'} top-0.5 right-[0.4%]  bg-gradient-to-tr from-slate-300 to-white h-screen  text-center  w-full py-20  `